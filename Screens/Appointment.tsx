...
const Appointment = ({navigation, route}) => {
  ...
  // Generate the next 30 days for the date picker
  const generateDates = () => {
    const dates = [];
...
<Text style = {{color : "gray"}}>{data.speciality}</Text>
          ...
          <View style={styles.consultationOptions}>
            <TouchableOpacity
              style={[
                styles.consultationBox,
                selectedConsultation === 'Chat' && styles.selectedConsultationBox,
              ]}
            ...
            <TouchableOpacity
              style={[
                ...
                selectedConsultation === 'Video' &&
                  styles.selectedConsultationBox,
              ]}
          ...
          <TouchableOpacity
            style={[
              ...
              !selectedConsultation && styles.disabledButton,
            ]}
            onPress={() => {dispatch(updateConsultationType(selectedConsultation));selectedConsultation && setStep(2)}}
          ...
          {Object.entries(timeSlots).map(([period, slots], index) => (
            <View key={index} style={styles.timeSlotSection}>
              <Text style={styles.timeSlotPeriod}>
                {period.charAt(0).toUpperCase() + period.slice(1)} (09AM - 12PM)
              </Text>
              <View style={styles.timeSlotContainer}>
                {slots.map((slot, idx) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.timeSlotBox,
                      selectedTime === slot && styles.selectedTimeSlotBox,
                    ]}
...
<TouchableOpacity
  ...
  onPress={() => {
    console.log(
      `Consultation: ${selectedConsultation}, Date: ${selectedDate}, Time: ${selectedTime}`
    );
    dispatch(updateAppointmentDateTime({ date: selectedDate, time: selectedTime }));;
    console.log(currentAppoitnment.id)
